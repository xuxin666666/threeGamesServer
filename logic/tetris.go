...
func TetrisGetScores(uid int64) (scores string, err error) {
	var exist bool
	exist, err = mysql.TetrisJudgeExist(uid)
	if err != nil {
		return
	}
	if exist {
		return mysql.TetrisGetScores(uid)
	} else {
		scores = ""
		return
	}
...
func TetrisUpdateScores(uid int64, scores int) (err error) {
	var exist bool
	var resScores string
	exist, err = mysql.TetrisJudgeExist(uid) // 判断该用户是否存过得分
	if err != nil {
		return
	}
	...
	if exist {
		resScores, err = mysql.TetrisGetScores(uid)
		if err != nil {
			return
		}
		...
		if scoreArrLen == 1 { // 如果记录为空
			err = mysql.TetrisUpdateScores(uid, newScore)
			if err != nil {
				return
			}
			...
			err = mysql.TetrisUpdateScores(uid, newScores)
			if err != nil {
				return
			}
		...
		err = mysql.TetrisInsertScores(uid, newScore)
		if err != nil {
			return
		}
	}
...
func TetrisUpdateScoreList(uid int64, scores string) (resScores string, err error) {
	var exist bool
	exist, err = mysql.TetrisJudgeExist(uid) // 判断该用户是否存过得分
	if err != nil {
		return
	...
	var scoreArr [][2]int
	scoreArr, err = handleScores(scores)
	if err != nil {
		return
	}
	...
	if exist {
		...
		if resScoresArrLen == 0 {
			...
			err = mysql.TetrisUpdateScores(uid, resScores)
			if err != nil {
				return
			}
		}
	} else {
		err = mysql.TetrisInsertScores(uid, scores)
		if err != nil {
			return
		}
	}
...
// 对scores进行格式检验，并根据分数从大到小排序
func handleScores(scores string) (scoreArr [][2]int, err error) {
	if scores == "" {
		return
	}
	var match bool
